...
import { presentSimple, usedTo } from "../data/Questions";
...
function QuestionScreen({route, navigation}) {
    ...
    if(questionName ==="Present Simple"){
        var questionPack = presentSimple;
    }else if(questionName ==="Used To"){
        var questionPack = usedTo;
    }
...
return(
    <View style={styles.QuestionScreenBg}>
        <View>
            ...
            <Ionicons name="arrow-back" color="black" size={30} onPress={()=> {navigation.goBack()}}/>
            <Text style={{marginRight: 95, fontSize: 20}}>{questionPack.testName}</Text>
            </View>
            ...
            {showScore ? (
                <View>
                    <View style={styles.answerSection}>
                        <Text style={styles.answerTextOne}>Sizning Ballingiz</Text>
                        <Text style={styles.answerTextTwo}>{score}/{presentSimple.length}</Text>
                    </View>
                ...
                <View>
                    <Text style={styles.questionP}>{questionPack[currentQuestion].questionText}</Text>
                </View>
                <View style={{marginTop: 40}}>
                          {questionPack[currentQuestion].answerOptions.map((answerOption)=>(
                            <TouchableOpacity  
